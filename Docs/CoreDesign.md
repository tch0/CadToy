...
- 目前的实体数据缓存顶点使用`float/glm::vec3`保存，低版本无法原生在shader中使用double类型，且double类型在显卡中计算性能很差，渲染时使用float并通过一系列手段控制误差是标准做法。对于无限实体构造线和射线直接生成大顶点数据就会出现精度问题，需要使用视口裁剪控制显示误差。
